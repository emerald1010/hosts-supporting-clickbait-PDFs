    ...
    def resolveReferences(self):
        ...
        if self.isEncodedStream:
            ...
            if not self.isFaultyDecoding():
                ...
                if numbers != [] and len(numbers) % 2 == 0:
                    for i in range(0,len(numbers),2):
                        # modified code to make it similar to how it is used elsewhere in the class
                        id = int(numbers[i])
                        offset = int(numbers[i+1])
                        ret = PDFParser().readObject(objectsSection[offset:])
                        if ret[0] == -1:
...
class PDFParser :
    ...
    def parsePDFSections(self, content, forceMode = False, looseMode = False):
        ...
        match_indexTrailer = re.search(r'\s+?(trailer)\s+?', content)
        if match_indexTrailer:
            indexTrailer = match_indexTrailer.start(1)
        else:
            indexTrailer = -1
        if indexTrailer != -1:
