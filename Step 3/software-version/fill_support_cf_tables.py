...
import log
logger = log.getlogger('cf_analyses_support', level=log.DEBUG, filename='/path/to/logs/cf_analyses.log')
...
def unfold_predictions(row):
    ...
    detected_components = row.iloc[2:]
...
def persist(df):
    engine = create_engine('postgresql://{user}:{password}@{host}:{port}/{dbname}'
                           .format(dbname=db_bindings['databases']['pipeline'], user=db_bindings['users']['user'],
                                   password=db_bindings['passwords']['password'],
                                   host=db_bindings['host'], port=db_bindings['port']), pool_pre_ping=True)

    try:
        df.to_sql('support_cfan_components', con=engine, if_exists='append', index=False, chunksize=500, method='multi')
...
def first_run(db_bindings):
    ...
    df = pd.DataFrame(scan_results, columns=['domain', 'page_data'])
        ...
        with conn.cursor() as cur:
            try:
                cur.execute("""
                    select domain, page_data_onl
                    from cf_analyses
                    where timestamp_onl > current_date - interval '1 day' + time '14:30' 
                    OR timestamp_off > current_date - interval '1 day' + time '14:30';""")
                scan_results = cur.fetchall()
            except psycopg2.errors.Error as e:
                logger.error(e)
                sys.exit(-1)
